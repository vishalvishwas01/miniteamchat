...
export default function MessageItem({ message, compact = false }) {
  ...
  return (
    <div className={`flex items-start gap-3 ${compact ? "mt-1" : "my-2"} ${isCurrent ? "justify-start" : "justify-end"}`}>
      {isCurrent ? (
        <>
          ...
          <div className="flex-1 max-w-xl">
            {!compact && <div className="text-sm font-semibold">{senderName} <span className="text-xs text-slate-400">· {time}</span></div>}
            <div className="mt-1">
        ...
        // other user's message aligned right
        <>
